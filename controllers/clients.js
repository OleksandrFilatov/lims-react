...
exports.deleteClient = async function (req, res) {
  try {
    ...
    result = await InputLab.find({ $or: [{ client: mongoose.Types.ObjectId(id) }, { material_category: mongoose.Types.ObjectId(id) }] })
    const _available3 = result.length === 0;

    if (!_available1 || !_available2 || !_available3) return res.status(400).json({ message: 'Already used in material or certificate type or input laboratory' });
