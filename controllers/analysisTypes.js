...
exports.dragAnalysisType = async (req, res) => {
  try {
    const objectives = await Objective.find();
    const units = await Unit.find();
    const aType = await AnalysisType.findById(req.body.id);

    var temp = [...aType.objectives];
    swap(temp, req.body.sourceIndex, req.body.destIndex);

    aType.objectives = temp;
    await aType.save();

    return res.json({ aType, objectives, units });
  } catch (err) {
